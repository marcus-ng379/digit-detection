...
void NetworkTrainer::StartTrainingSession(int num_epochs) {
    ...
    for (int epoch = 1; epoch <= num_epochs; epoch++) {
        ...
        this->epoch = epoch;
        // Evaluation
        EvaluationData* train_eval = this->evaluator.Evaluate(neural_network, this->training_data, this->trainin_data_length);
...
void NetworkTrainer::LoadData() {
    ...
    this->trainin_data_length = result.first.second;
    this->validation_data = result.second.first;
    this->validation_data_length = result.second.second;

    std::pair<Batch**, int> result1 = this->data_helper.CreateMiniBatches(this->training_data, this->trainin_data_length, this->network_settings->get_mini_batch_size());
